...
const require_src = require('./src-3e3h_kFE.cjs');
let _modelcontextprotocol_client__shims = require("@modelcontextprotocol/client/_shims");
let pkce_challenge = require("pkce-challenge");
pkce_challenge = require_chunk.__toESM(pkce_challenge);
...
async function resolveAuthorizationCallbackParams(codeOrParams, iss, provider, serverUrl, opts) {
	...
	if (error) throw new require_src.OAuthError(error, codeOrParams.get("error_description") ?? error, codeOrParams.get("error_uri") ?? void 0);
...
async function parseErrorResponse(input) {
	...
	try {
		const result = require_src.OAuthErrorResponseSchema.parse(JSON.parse(body));
		return require_src.OAuthError.fromResponse(result);
	} catch (error) {
		const errorMessage = `${statusCode ? `HTTP ${statusCode}: ` : ""}Invalid OAuth error response: ${error}. Raw body: ${body}`;
		return new require_src.OAuthError(require_src.OAuthErrorCode.ServerError, errorMessage);
	}
}
/**
...
async function auth(provider, options) {
	try {
		...
		if (error instanceof require_src.OAuthError) {
			if (error.code === require_src.OAuthErrorCode.InvalidClient || error.code === require_src.OAuthErrorCode.UnauthorizedClient) {
				await provider.invalidateCredentials?.("client");
				await provider.invalidateCredentials?.("tokens");
				return await authInternal(provider, options);
			} else if (error.code === require_src.OAuthErrorCode.InvalidGrant) {
				await provider.invalidateCredentials?.("tokens");
				return await authInternal(provider, options);
			}
		}
...
async function authInternal(provider, { serverUrl, authorizationCode, iss, scope, resourceMetadataUrl, fetchFn, skipIssuerMetadataValidation, forceReauthorization }) {
	...
	if (!clientInformation) {
		...
		if (clientMetadataUrl && !isHttpsUrl(clientMetadataUrl)) throw new require_src.OAuthError(require_src.OAuthErrorCode.InvalidClientMetadata, `clientMetadataUrl must be a valid HTTPS URL with a non-root pathname, got: ${clientMetadataUrl}`);
...
function validateClientMetadataUrl(url) {
	if (url && !isHttpsUrl(url)) throw new require_src.OAuthError(require_src.OAuthErrorCode.InvalidClientMetadata, `clientMetadataUrl must be a valid HTTPS URL with a non-root pathname, got: ${url}`);
}
/**
...
async function selectResourceURL(serverUrl, provider, resourceMetadata) {
	const defaultResource = require_src.resourceUrlFromServerUrl(serverUrl);
	...
	if (!require_src.checkResourceAllowed({
...
async function discoverOAuthProtectedResourceMetadata(serverUrl, opts, fetchFn = fetch) {
	...
	return require_src.OAuthProtectedResourceMetadataSchema.parse(await response.json());
}
/**
...
async function fetchWithCorsRetry(url, headers, fetchFn = fetch) {
	try {
		...
		if (!(error instanceof TypeError) || !_modelcontextprotocol_client__shims.CORS_IS_POSSIBLE) throw error;
		if (headers) try {
...
async function discoverMetadataWithFallback(serverUrl, wellKnownType, fetchFn, opts) {
	...
	const protocolVersion = opts?.protocolVersion ?? require_src.LATEST_PROTOCOL_VERSION;
	let url;
...
async function discoverOAuthMetadata(issuer, { authorizationServerUrl, protocolVersion } = {}, fetchFn = fetch) {
	...
	protocolVersion ??= require_src.LATEST_PROTOCOL_VERSION;
	...
	return require_src.OAuthMetadataSchema.parse(await response.json());
}
/**
...
*/
async function discoverAuthorizationServerMetadata(authorizationServerUrl, { fetchFn = fetch, protocolVersion = require_src.LATEST_PROTOCOL_VERSION, skipIssuerValidation = false } = {}) {
	const headers = {
 ...
 /**
		...
		*/
		...
		const parsed = type === "oauth" ? require_src.OAuthMetadataSchema.parse(await response.json()) : require_src.OpenIdProviderDiscoveryMetadataSchema.parse(await response.json());
...
async function startAuthorization(authorizationServerUrl, { metadata, clientInformation, redirectUrl, scope, state, resource }) {
	...
	const challenge = await (0, pkce_challenge.default)();
...
async function executeTokenRequest(authorizationServerUrl, { metadata, tokenRequestParams, clientInformation, addClientAuthentication, resource, fetchFn }) {
	...
	try {
		return require_src.OAuthTokensSchema.parse(json);
	} catch (parseError) {
...
async function registerClient(authorizationServerUrl, { metadata, clientMetadata, scope, fetchFn }) {
	...
	return require_src.OAuthClientInformationFullSchema.parse(await response.json());
}
	...
	*/
	async toolDefinition(name) {
		...
		if (this._toolIndex?.stamp !== entry.stamp) {
			...
			for (const tool of entry.value.tools) byName.set(tool.name, tool);
			this._toolIndex = {
				stamp: entry.stamp,
				byName
			};
...
function classifyProbeOutcome(outcome, context) {
	switch (outcome.kind) {
		...
		case "timeout":
			...
			return {
				kind: "error",
				error: new require_src.SdkError(require_src.SdkErrorCode.RequestTimeout, `Version negotiation probe timed out after ${outcome.timeoutMs}ms`, { timeout: outcome.timeoutMs })
			};
	}
}
function classifyResult(result, context) {
	const parsed = require_src.codecForVersion(require_src.MODERN_WIRE_REVISION).validateResult("server/discover", result);
...
function classifyRpcError(outcome, context) {
	...
	if (code === UNSUPPORTED_PROTOCOL_VERSION) {
		...
		const supportedModern = require_src.modernProtocolVersions(supported);
...
function classifyNetworkError(error, context) {
	...
	return {
		kind: "error",
		error: new require_src.SdkError(require_src.SdkErrorCode.EraNegotiationFailed, `Version negotiation probe failed: ${describeError(error)}`, { cause: error })
	};
}
...
function resolveVersionNegotiation(options, supportedProtocolVersionsOption) {
	...
	if (typeof mode === "object") {
		if (!require_src.isModernProtocolVersion(mode.pin)) throw new TypeError(`versionNegotiation: { pin: '${mode.pin}' } is not a modern protocol revision — pinning is for 2026-07-28 and later; omit versionNegotiation (or use mode: 'legacy') for 2025-era servers.`);
		return {
			kind: "pin",
			version: mode.pin,
			probe
	...
	const explicitModern = supportedProtocolVersionsOption ? require_src.modernProtocolVersions(supportedProtocolVersionsOption) : [];
	return {
		kind: "auto",
		modernVersions: explicitModern.length > 0 ? explicitModern : [...require_src.SUPPORTED_MODERN_PROTOCOL_VERSIONS],
		fallbackAvailable: supportedProtocolVersionsOption ? require_src.legacyProtocolVersions(supportedProtocolVersionsOption).length > 0 : true,
		probe
	};
}
...
var ProbeWindow = class ProbeWindow {
	...
	static async open(transport) {
		...
		transport.onmessage = (message) => {
			...
			if (pending !== void 0 && (require_src.isJSONRPCResultResponse(message) || require_src.isJSONRPCErrorResponse(message)) && message.id === pending.id) {
				window._pending = void 0;
				if (require_src.isJSONRPCResultResponse(message)) pending.resolve({
					kind: "response",
	...
	*/
	...
	detach() {
		...
		this._transport.onerror = void 0;
...
function buildProbeRequest(id, protocolVersion, clientInfo, capabilities) {
	return {
		...
		params: { _meta: require_src.codecForVersion(protocolVersion).outboundEnvelope({
			protocolVersion,
			clientInfo,
...
function normalizeReply(reply, timeoutMs) {
	switch (reply.kind) {
		...
		case "send-error": {
			...
			if (error instanceof require_src.SdkHttpError) {
		...
		case "closed": return {
			kind: "network-error",
			error: /* @__PURE__ */ new Error("Connection closed during the version negotiation probe")
		};
		case "timeout": return {
			kind: "timeout",
			timeoutMs
...
async function negotiateEra(negotiation, deps) {
	...
	const probe = async () => {
		...
		for (;;) {
			...
			const verdict = classifyProbeOutcome(normalizeReply(reply, timeoutMs), {
			...
			switch (verdict.kind) {
				...
				case "corrective":
					...
					continue;
				case "legacy":
					if (negotiation.kind === "pin") throw new require_src.SdkError(require_src.SdkErrorCode.EraNegotiationFailed, `Version negotiation failed: the server did not offer pinned protocol version ${negotiation.version} via server/discover (no fallback in pin mode)`);
					if (!negotiation.fallbackAvailable) throw new require_src.SdkError(require_src.SdkErrorCode.EraNegotiationFailed, "Version negotiation failed: the server gave no modern evidence and this client supports no pre-2026-07-28 protocol version to fall back to");
					return { era: "legacy" };
...
var Client = class extends require_src.Protocol {
	...
	*/
	_resetConnectionState() {
		...
		if (this._listenState.size > 0) {
			const reason = new require_src.SdkError(require_src.SdkErrorCode.ConnectionClosed, "subscriptions/listen: client reconnected or closed; subscription state from the previous connection was reset");
	...
	*/
	constructor(_clientInfo, options) {
		...
		this._jsonSchemaValidator = options?.jsonSchemaValidator ?? new _modelcontextprotocol_client__shims.DefaultJsonSchemaValidator();
		...
		this._inputRequiredDriverConfig = require_src.resolveInputRequiredDriverConfig(options?.inputRequired);
	...
	*/
	_shouldDropInbound(message) {
		if (this._negotiatedProtocolVersion !== void 0 && require_src.isModernProtocolVersion(this._negotiatedProtocolVersion) && require_src.isJSONRPCRequest(message)) return "drop";
	}
	/**
	...
	*/
	_resolveNonCompleteResult(decoded, flow) {
		if (!this._inputRequiredDriverConfig.autoFulfill) return Promise.reject(new require_src.SdkError(require_src.SdkErrorCode.UnsupportedResultType, `Unsupported result type 'input_required' for ${flow.request.method}: multi-round-trip auto-fulfilment is not enabled on this instance — pass allowInputRequired: true to handle it manually, or enable inputRequired.autoFulfill`, {
			resultType: "input_required",
			method: flow.request.method
		}));
		return require_src.runInputRequiredFlow({
	...
	*/
	_wrapHandler(method, handler) {
		if (method === "elicitation/create") return async (request, ctx) => {
			...
			if (!validatedRequest.ok) throw new require_src.ProtocolError(validatedRequest.reason === "not-in-era" ? require_src.ProtocolErrorCode.InternalError : require_src.ProtocolErrorCode.InvalidParams, validatedRequest.reason === "not-in-era" ? "No wire schema for elicitation/create in the resolved era" : `Invalid elicitation request: ${validatedRequest.message}`);
			const { params } = validatedRequest.value;
			params.mode = params.mode ?? "form";
			const { supportsFormMode, supportsUrlMode } = getSupportedElicitationModes(this._capabilities.elicitation);
			if (params.mode === "form" && !supportsFormMode) throw new require_src.ProtocolError(require_src.ProtocolErrorCode.InvalidParams, "Client does not support form-mode elicitation requests");
			if (params.mode === "url" && !supportsUrlMode) throw new require_src.ProtocolError(require_src.ProtocolErrorCode.InvalidParams, "Client does not support URL-mode elicitation requests");
			...
			if (!validationResult.ok) throw new require_src.ProtocolError(validationResult.reason === "not-in-era" ? require_src.ProtocolErrorCode.InternalError : require_src.ProtocolErrorCode.InvalidParams, validationResult.reason === "not-in-era" ? "No wire schema for elicitation/create in the resolved era" : `Invalid elicitation result: ${validationResult.message}`);
		...
		if (method === "sampling/createMessage") return async (request, ctx) => {
			...
			if (!validatedRequest.ok) throw new require_src.ProtocolError(validatedRequest.reason === "not-in-era" ? require_src.ProtocolErrorCode.InternalError : require_src.ProtocolErrorCode.InvalidParams, validatedRequest.reason === "not-in-era" ? "No wire schema for sampling/createMessage in the resolved era" : `Invalid sampling request: ${validatedRequest.message}`);
			const { params } = validatedRequest.value;
			const result = await handler(request, ctx);
			...
			if (!validatedResult.ok) throw new require_src.ProtocolError(validatedResult.reason === "not-in-era" ? require_src.ProtocolErrorCode.InternalError : require_src.ProtocolErrorCode.InvalidParams, validatedResult.reason === "not-in-era" ? "No result schema for sampling/createMessage in the resolved era" : `Invalid sampling result: ${validatedResult.message}`);
			return validatedResult.value;
		};
		return handler;
	}
	assertCapability(capability, method) {
		if (!this._serverCapabilities?.[capability]) throw new require_src.SdkError(require_src.SdkErrorCode.CapabilityNotSupported, `Server does not support ${capability} (required for ${method})`);
	}
	/**
	...
	*/
	async connect(transport, options) {
		if (options?.prior !== void 0) return this._connectFromPrior(transport, options.prior);
	...
	*/
	async _legacyHandshake(transport, options) {
		...
		try {
			...
			if (offeredVersion === void 0) throw new require_src.SdkError(require_src.SdkErrorCode.EraNegotiationFailed, "Cannot run the initialize handshake: supportedProtocolVersions contains no pre-2026-07-28 protocol version");
	...
	*/
	async _connectFromPrior(transport, prior) {
		this._resetConnectionState();
		const explicit = this._supportedProtocolVersionsOption;
		const version = (explicit && require_src.modernProtocolVersions(explicit).length > 0 ? require_src.modernProtocolVersions(explicit) : require_src.SUPPORTED_MODERN_PROTOCOL_VERSIONS).find((v) => prior.supportedVersions.includes(v));
		if (version === void 0) throw new require_src.SdkError(require_src.SdkErrorCode.EraNegotiationFailed, "connect({ prior }) requires a 2026-07-28+ mutual protocol version; the supplied DiscoverResult and this client's supportedProtocolVersions have no modern overlap. Use versionNegotiation: { mode: 'auto' } for legacy-era fallback.");
		await super.connect(transport);
	...
	*/
	_deriveServerIdentity(transport) {
		...
		return v === void 0 ? transport.sessionId ?? "" : `${v.name}@${v.version}`;
	}
	/**
	...
	*/
	getProtocolEra() {
		const version = this._negotiatedProtocolVersion;
		if (version === void 0) return void 0;
		return require_src.isModernProtocolVersion(version) ? "modern" : "legacy";
	}
	/**
	...
	assertCapabilityForMethod(method) {
		switch (method) {
			case "logging/setLevel":
				if (!this._serverCapabilities?.logging) throw new require_src.SdkError(require_src.SdkErrorCode.CapabilityNotSupported, `Server does not support logging (required for ${method})`);
				break;
			case "prompts/get":
			case "prompts/list":
				if (!this._serverCapabilities?.prompts) throw new require_src.SdkError(require_src.SdkErrorCode.CapabilityNotSupported, `Server does not support prompts (required for ${method})`);
				break;
			case "resources/list":
			...
			case "resources/unsubscribe":
				if (!this._serverCapabilities?.resources) throw new require_src.SdkError(require_src.SdkErrorCode.CapabilityNotSupported, `Server does not support resources (required for ${method})`);
				if (method === "resources/subscribe" && !this._serverCapabilities.resources.subscribe) throw new require_src.SdkError(require_src.SdkErrorCode.CapabilityNotSupported, `Server does not support resource subscriptions (required for ${method})`);
				break;
			case "tools/call":
			case "tools/list":
				if (!this._serverCapabilities?.tools) throw new require_src.SdkError(require_src.SdkErrorCode.CapabilityNotSupported, `Server does not support tools (required for ${method})`);
				break;
			case "completion/complete":
				if (!this._serverCapabilities?.completions) throw new require_src.SdkError(require_src.SdkErrorCode.CapabilityNotSupported, `Server does not support completions (required for ${method})`);
				break;
	...
	assertNotificationCapability(method) {
		switch (method) {
			case "notifications/roots/list_changed":
				if (!this._capabilities.roots?.listChanged) throw new require_src.SdkError(require_src.SdkErrorCode.CapabilityNotSupported, `Client does not support roots list changed notifications (required for ${method})`);
				break;
	...
	assertRequestHandlerCapability(method) {
		switch (method) {
			case "sampling/createMessage":
				if (!this._capabilities.sampling) throw new require_src.SdkError(require_src.SdkErrorCode.CapabilityNotSupported, `Client does not support sampling capability (required for ${method})`);
				break;
			case "elicitation/create":
				if (!this._capabilities.elicitation) throw new require_src.SdkError(require_src.SdkErrorCode.CapabilityNotSupported, `Client does not support elicitation capability (required for ${method})`);
				break;
			case "roots/list":
				if (!this._capabilities.roots) throw new require_src.SdkError(require_src.SdkErrorCode.CapabilityNotSupported, `Client does not support roots capability (required for ${method})`);
				break;
			case "ping": break;
		}
	}
	...
	*/
	async discover(options) {
		const result = await this._requestWithSchema({ method: "server/discover" }, require_src.DiscoverResultSchema, options);
	...
	*/
	async _listAllPages(method, baseParams, options, append, finalize) {
		...
		while (cursor !== void 0 && !seen.has(cursor)) {
			if (this._listMaxPages !== 0 && pages >= this._listMaxPages) throw new require_src.SdkError(require_src.SdkErrorCode.ListPaginationExceeded, `${method}: exceeded listMaxPages (${this._listMaxPages}); server pagination did not terminate`, {
				method,
	...
	*/
	async _serveFromCache(method, params, options) {
		...
		if (entry?.expiresAt !== void 0 && entry.expiresAt > this._cache.now()) {
			if (options?.signal?.aborted) {
				const reason = options.signal.reason;
				throw reason instanceof require_src.SdkError ? reason : new require_src.SdkError(require_src.SdkErrorCode.RequestTimeout, String(reason));
			}
	...
	*/
	async _resolveXMcpHeaderScan(name, override) {
		...
		return tool === void 0 ? void 0 : require_src.scanXMcpHeaderDeclarations(tool.inputSchema);
	}
	/**
	...
	*/
	async listen(filter, options) {
		if (this.transport === void 0) throw new require_src.SdkError(require_src.SdkErrorCode.NotConnected, "Not connected");
		const negotiated = this._negotiatedProtocolVersion;
		if (negotiated === void 0 || !require_src.isModernProtocolVersion(negotiated)) throw new require_src.SdkError(require_src.SdkErrorCode.MethodNotSupportedByProtocolVersion, `subscriptions/listen requires a 2026-07-28-era connection (negotiated: ${negotiated ?? "none"}). On a 2025-era connection, change notifications are delivered unsolicited: use ClientOptions.listChanged and resources/subscribe instead.`, {
		...
		const settle = (outcome) => {
			...
			if (wasOpening) rejectOpening(outcome.error ?? new require_src.SdkError(require_src.SdkErrorCode.ConnectionClosed, "subscriptions/listen closed before the server acknowledged"));
		};
		...
		const ackTimeout = options?.timeout ?? require_src.DEFAULT_REQUEST_TIMEOUT_MSEC;
		ackTimer = setTimeout(() => {
			settle({
				cause: "remote",
				error: new require_src.SdkError(require_src.SdkErrorCode.RequestTimeout, "subscriptions/listen ack timed out", { timeout: ackTimeout })
			});
			wireTeardown().catch(() => {});
		}, ackTimeout);
		if (options?.signal) {
	...
	*/
	_onnotification(raw, extra) {
		...
		if (raw.method === "notifications/subscriptions/acknowledged") {
			const subscriptionId = raw.params?._meta?.[require_src.SUBSCRIPTION_ID_META_KEY];
	...
	*/
	_onresponse(response) {
		...
		if (entry !== void 0) {
			if (require_src.isJSONRPCErrorResponse(response)) entry.settle({
				cause: "remote",
				error: require_src.ProtocolError.fromError(response.error.code, response.error.message, response.error.data)
			});
			else entry.settle({
				cause: "graceful",
				error: new require_src.SdkError(require_src.SdkErrorCode.ConnectionClosed, "subscriptions/listen: server closed the subscription gracefully before acknowledging")
			});
			return;
		}
	...
	*/
	_onclose() {
		if (this._listenState.size > 0) {
			const reason = new require_src.SdkError(require_src.SdkErrorCode.ConnectionClosed, "Connection closed");
			for (const entry of this._listenState.values()) entry.settle({
				cause: "remote",
				error: reason
			});
	...
	*/
	async callTool(params, options) {
		...
		const buildSendOptions = async () => {
			...
			const paramHeaders = require_src.buildMcpParamHeaders(scan.declarations, params.arguments);
		...
		const assertCompiled = () => {
			...
			throw new require_src.ProtocolError(require_src.ProtocolErrorCode.InvalidParams, `Tool '${params.name}' has an invalid outputSchema: ${message}`);
		};
		assertCompiled();
		let result;
		try {
			...
			const isHeaderMismatch = error instanceof require_src.ProtocolError && error.code === require_src.HEADER_MISMATCH_ERROR_CODE;
		...
		if (validator) {
			if (result.structuredContent === void 0 && !result.isError) throw new require_src.ProtocolError(require_src.ProtocolErrorCode.InvalidRequest, `Tool ${params.name} has an output schema but did not return structured content`);
			if (result.structuredContent !== void 0 && !result.isError) try {
				const validationResult = validator(result.structuredContent);
				if (!validationResult.valid) throw new require_src.ProtocolError(require_src.ProtocolErrorCode.InvalidParams, `Structured content does not match the tool's output schema: ${validationResult.errorMessage}`);
			} catch (error) {
				if (error instanceof require_src.ProtocolError) throw error;
				throw new require_src.ProtocolError(require_src.ProtocolErrorCode.InvalidParams, `Failed to validate structured content: ${error instanceof Error ? error.message : String(error)}`);
			}
		}
		return result;
	}
	...
	*/
	_excludeInvalidXMcpHeaderTools(result) {
		...
		const filtered = result.tools.filter((tool) => {
			const scan = require_src.scanXMcpHeaderDeclarations(tool.inputSchema);
			if (!scan.valid) {
	...
	*/
	_setupListChangedHandler(listType, notificationMethod, options, fetcher) {
		const parseResult = require_src.parseSchema(require_src.ListChangedOptionsBaseSchema, options);
...
async function requestJwtAuthorizationGrant(options) {
	...
	if (!response.ok) {
		...
		const parseResult$1 = require_src.OAuthErrorResponseSchema.safeParse(errorBody);
	...
	const parseResult = require_src.IdJagTokenExchangeResponseSchema.safeParse(await response.json());
...
async function exchangeJwtAuthGrant(options) {
	...
	const parseResult = require_src.OAuthTokensSchema.safeParse(responseBody);
...
var SSEClientTransport = class {
	...
	async _commonHeaders() {
		...
		const extraHeaders = require_src.normalizeHeaders(this._requestInit?.headers);
		return new Headers({
			...headers,
			...extraHeaders
		});
	...
	_startOrAuth() {
		...
		return new Promise((resolve, reject) => {
			this._eventSource = new eventsource.EventSource(this._url.href, {
			...
			this._eventSource.onmessage = (event) => {
				...
				let message;
				try {
					message = require_src.JSONRPCMessageSchema.parse(JSON.parse(messageEvent.data));
				} catch (error) {
					this.onerror?.(error);
					return;
				}
	...
	async _send(message, isAuthRetry) {
		if (!this._endpoint) throw new require_src.SdkError(require_src.SdkErrorCode.NotConnected, "Not connected");
		try {
			const headers = await this._commonHeaders();
...
var StreamableHTTPClientTransport = class {
	...
	*/
	async _stepUpAuthorize(challenge, stepUpRetries) {
		...
		if (stepUpRetries >= this._maxStepUpRetries) throw new require_src.SdkHttpError(require_src.SdkErrorCode.ClientHttpForbidden, `Server returned 403 insufficient_scope after step-up re-authorization (retry limit ${this._maxStepUpRetries} reached)`, {
			status: 403,
	...
	*/
	_applyBodyDerivedHeaders(headers, message) {
		if (Array.isArray(message) || !require_src.isJSONRPCRequest(message)) return;
		const envelopeVersion = (message.params?._meta)?.[require_src.PROTOCOL_VERSION_META_KEY];
		...
		if (nameHeader !== void 0) headers.set("mcp-name", require_src.encodeMcpParamValue(nameHeader));
	}
	/**
	...
	*/
	_isModernEnvelopedRequest(message) {
		if (Array.isArray(message) || !require_src.isJSONRPCRequest(message)) return false;
		const v = (message.params?._meta)?.[require_src.PROTOCOL_VERSION_META_KEY];
		return typeof v === "string" && require_src.isModernProtocolVersion(v);
	}
	async _startOrAuthSse(options, isAuthRetry = false, stepUpRetries = 0) {
		...
		try {
			...
			if (!response.ok) {
				...
				throw new require_src.SdkHttpError(require_src.SdkErrorCode.ClientHttpFailedToOpenStream, `Failed to open SSE stream: ${response.statusText}`, {
					status: response.status,
					statusText: response.statusText
				});
			}
	...
	*/
	...
	_handleSseStream(stream, options, isReconnectable) {
		...
		const processStream = async () => {
			try {
				const reader = stream.pipeThrough(new TextDecoderStream()).pipeThrough(new eventsource_parser_stream.EventSourceParserStream({ onRetry: (retryMs) => {
				...
				while (true) {
					...
					if (!event.event || event.event === "message") try {
						const message = require_src.JSONRPCMessageSchema.parse(JSON.parse(event.data));
		...
		try {
			...
			if (resumptionToken) {
				this._startOrAuthSse({
					resumptionToken,
					replayMessageId: require_src.isJSONRPCRequest(message) ? message.id : void 0,
			...
			if (!response.ok) {
				...
				if (response.status === 400 && typeof text === "string" && this._isModernEnvelopedRequest(message)) try {
					const parsed = require_src.JSONRPCMessageSchema.parse(JSON.parse(text));
					const requests = (Array.isArray(message) ? message : [message]).filter((m) => require_src.isJSONRPCRequest(m));
					if (require_src.isJSONRPCErrorResponse(parsed) && requests.some((r) => r.id === parsed.id)) {
						this.onmessage?.(parsed);
						return;
					}
				} catch {}
				throw new require_src.SdkHttpError(require_src.SdkErrorCode.ClientHttpNotImplemented, `Error POSTing to endpoint: ${text}`, {
					status: response.status,
					statusText: response.statusText,
					text
				});
			...
			if (hasRequests) if (contentType?.includes("text/event-stream")) this._handleSseStream(response.body, {
				onresumptiontoken,
				requestSignal: options?.requestSignal,
				...
				const responseMessages = Array.isArray(data) ? data.map((msg) => require_src.JSONRPCMessageSchema.parse(msg)) : [require_src.JSONRPCMessageSchema.parse(data)];
	...
	*/
	async terminateSession() {
		...
		try {
			...
			const response = await (this._fetch ?? fetch)(this._url, init);
			await response.text?.().catch(() => {});
			if (!response.ok && response.status !== 405) throw new require_src.SdkHttpError(require_src.SdkErrorCode.ClientHttpFailedToTerminateSession, `Failed to terminate session: ${response.statusText}`, {
				status: response.status,
				statusText: response.statusText
			});
...
function fromJsonSchema(schema, validator) {
	return require_src.fromJsonSchema(schema, validator ?? (_defaultValidator ??= new _modelcontextprotocol_client__shims.DefaultJsonSchemaValidator()));
}

//#endregion
exports.AuthorizationServerMismatchError = AuthorizationServerMismatchError;
exports.BAGGAGE_META_KEY = require_src.BAGGAGE_META_KEY;
exports.CLIENT_CAPABILITIES_META_KEY = require_src.CLIENT_CAPABILITIES_META_KEY;
exports.CLIENT_INFO_META_KEY = require_src.CLIENT_INFO_META_KEY;
exports.Client = Client;
exports.ClientCredentialsProvider = ClientCredentialsProvider;
exports.CrossAppAccessProvider = CrossAppAccessProvider;
exports.DEFAULT_NEGOTIATED_PROTOCOL_VERSION = require_src.DEFAULT_NEGOTIATED_PROTOCOL_VERSION;
exports.DEFAULT_REQUEST_TIMEOUT_MSEC = require_src.DEFAULT_REQUEST_TIMEOUT_MSEC;
exports.INTERNAL_ERROR = require_src.INTERNAL_ERROR;
exports.INVALID_PARAMS = require_src.INVALID_PARAMS;
exports.INVALID_REQUEST = require_src.INVALID_REQUEST;
exports.InMemoryResponseCacheStore = InMemoryResponseCacheStore;
exports.InMemoryTransport = require_src.InMemoryTransport;
exports.InsecureTokenEndpointError = InsecureTokenEndpointError;
exports.InsufficientScopeError = InsufficientScopeError;
exports.IssuerMismatchError = IssuerMismatchError;
exports.JSONRPC_VERSION = require_src.JSONRPC_VERSION;
exports.LATEST_PROTOCOL_VERSION = require_src.LATEST_PROTOCOL_VERSION;
exports.LOG_LEVEL_META_KEY = require_src.LOG_LEVEL_META_KEY;
exports.MAX_CACHE_TTL_MS = MAX_CACHE_TTL_MS;
exports.METHOD_NOT_FOUND = require_src.METHOD_NOT_FOUND;
exports.MissingRequiredClientCapabilityError = require_src.MissingRequiredClientCapabilityError;
exports.OAuthClientFlowError = OAuthClientFlowError;
exports.OAuthError = require_src.OAuthError;
exports.OAuthErrorCode = require_src.OAuthErrorCode;
exports.PARSE_ERROR = require_src.PARSE_ERROR;
exports.PROTOCOL_VERSION_META_KEY = require_src.PROTOCOL_VERSION_META_KEY;
...
exports.RELATED_TASK_META_KEY = require_src.RELATED_TASK_META_KEY;
...
exports.SUBSCRIPTION_ID_META_KEY = require_src.SUBSCRIPTION_ID_META_KEY;
exports.SUPPORTED_PROTOCOL_VERSIONS = require_src.SUPPORTED_PROTOCOL_VERSIONS;
...
exports.TRACEPARENT_META_KEY = require_src.TRACEPARENT_META_KEY;
exports.TRACESTATE_META_KEY = require_src.TRACESTATE_META_KEY;
...
exports.discoverAndRequestJwtAuthGrant = discoverAndRequestJwtAuthGrant;
...
exports.isStrictScopeSuperset = isStrictScopeSuperset;
...
exports.resolveClientMetadata = resolveClientMetadata;
...
exports.validateAuthorizationResponseIssuer = validateAuthorizationResponseIssuer;
exports.validateClientMetadataUrl = validateClientMetadataUrl;
exports.withInputRequired = require_src.withInputRequired;
