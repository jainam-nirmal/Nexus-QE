import { $ as CLIENT_CAPABILITIES_META_KEY, A as isJSONRPCErrorResponse, B as MODERN_WIRE_REVISION, C as scanXMcpHeaderDeclarations, Ct as OAuthTokensSchema, D as isInitializeRequest, Dt as SdkHttpError, E as isCallToolResult, Et as SdkErrorCode, F as isTaskAugmentedRequestParams, G as UnsupportedProtocolVersionError, H as MissingRequiredClientCapabilityError, I as parseJSONRPCMessage, J as SUPPORTED_MODERN_PROTOCOL_VERSIONS, K as UrlElicitationRequiredError, L as DiscoverResultSchema, M as isJSONRPCRequest, N as isJSONRPCResponse, O as isInitializedNotification, Ot as OAuthError, P as isJSONRPCResultResponse, Q as BAGGAGE_META_KEY, R as JSONRPCMessageSchema, S as encodeMcpParamValue, St as OAuthProtectedResourceMetadataSchema, T as assertCompleteRequestResourceTemplate, Tt as SdkError, U as ProtocolError, V as codecForVersion, W as ResourceNotFoundError, X as legacyProtocolVersions, Y as isModernProtocolVersion, Z as modernProtocolVersions, _ as specTypeSchemas, _t as resourceUrlFromServerUrl, a as createFetchWithInit, at as JSONRPC_VERSION, b as HEADER_MISMATCH_ERROR_CODE, bt as OAuthErrorResponseSchema, c as STDIO_DEFAULT_MAX_BUFFER_SIZE, ct as METHOD_NOT_FOUND, d as getDisplayName, dt as RELATED_TASK_META_KEY, et as CLIENT_INFO_META_KEY, f as runInputRequiredFlow, ft as SUBSCRIPTION_ID_META_KEY, g as isSpecType, gt as checkResourceAllowed, h as mergeCapabilities, ht as TRACESTATE_META_KEY, i as UriTemplate, it as INVALID_REQUEST, j as isJSONRPCNotification, k as isInputRequiredResult, kt as OAuthErrorCode, l as deserializeMessage, lt as PARSE_ERROR, m as Protocol, mt as TRACEPARENT_META_KEY, n as parseSchema, nt as INTERNAL_ERROR, o as normalizeHeaders, ot as LATEST_PROTOCOL_VERSION, p as DEFAULT_REQUEST_TIMEOUT_MSEC, pt as SUPPORTED_PROTOCOL_VERSIONS, q as ProtocolErrorCode, r as InMemoryTransport, rt as INVALID_PARAMS, s as ReadBuffer, st as LOG_LEVEL_META_KEY, t as fromJsonSchema$1, tt as DEFAULT_NEGOTIATED_PROTOCOL_VERSION, u as serializeMessage, ut as PROTOCOL_VERSION_META_KEY, v as resolveInputRequiredDriverConfig, vt as IdJagTokenExchangeResponseSchema, w as assertCompleteRequestPrompt, wt as OpenIdProviderDiscoveryMetadataSchema, x as buildMcpParamHeaders, xt as OAuthMetadataSchema, y as withInputRequired, yt as OAuthClientInformationFullSchema, z as ListChangedOptionsBaseSchema } from "./src-Bpc1d5Jt.mjs";
		...
		for (;;) {
			...
			switch (verdict.kind) {
				...
				case "legacy":
					...
					if (!negotiation.fallbackAvailable) throw new SdkError(SdkErrorCode.EraNegotiationFailed, "Version negotiation failed: the server gave no modern evidence and this client supports no pre-2026-07-28 protocol version to fall back to");
					return { era: "legacy" };
...
var Client = class extends Protocol {
	...
	*/
	async _connectFromPrior(transport, prior) {
		...
		const version = (explicit && modernProtocolVersions(explicit).length > 0 ? modernProtocolVersions(explicit) : SUPPORTED_MODERN_PROTOCOL_VERSIONS).find((v) => prior.supportedVersions.includes(v));
		if (version === void 0) throw new SdkError(SdkErrorCode.EraNegotiationFailed, "connect({ prior }) requires a 2026-07-28+ mutual protocol version; the supplied DiscoverResult and this client's supportedProtocolVersions have no modern overlap. Use versionNegotiation: { mode: 'auto' } for legacy-era fallback.");
		await super.connect(transport);
